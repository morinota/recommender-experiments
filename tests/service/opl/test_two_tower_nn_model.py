...
def test_TwoTowerモデルが正しく初期化されること():
    ...
    assert context_output_layer.out_features == dim_two_tower_embedding, (
    ...
    assert action_output_layer.out_features == dim_two_tower_embedding, (
...
def test_アクション候補の数が動的に変化してもアクション選択の確率分布の推論ができること():
    ...
    assert action_dist.shape == (n_rounds, n_actions + 2), "出力の形状が(ラウンド数、アクション数)であること"
    assert np.allclose(action_dist.sum(axis=1), 1.0), "各ラウンドごとに、確率の総和が1.0"
...
def test_バンディットフィードバックデータを元にIPS推定量で勾配ベースのオフ方策学習ができること():
    ...
    sut = PolicyByTwoTowerModel(
        dim_context_features, dim_action_features, dim_two_tower_embedding, off_policy_objective=off_policy_objective
    )
