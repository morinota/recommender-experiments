...
class ContextGNNNetwork(nn.Module):
    ...
    def __init__(self, context_dim: int, action_dim: int, hidden_dim: int = 64, dropout_rate: float = 0.1, num_layers: int = 3):
        super().__init__()
    ...
    def forward(self, context: torch.Tensor, action_context: torch.Tensor) -> torch.Tensor:
        ...
        action_for_attention = action_encoded.unsqueeze(0).expand(batch_size, -1, -1)  # (batch_size, n_actions, hidden_dim)
        ...
        ], dim=-1)  # (batch_size, n_actions, hidden_dim * 3)
...
class PolicyByContextGnn(PolicyStrategyInterface):
    ...
    def fit(
        ...
        self.model = ContextGNNNetwork(
            context_dim, action_dim, self.hidden_dim, 
