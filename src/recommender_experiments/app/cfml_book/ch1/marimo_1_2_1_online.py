...
@app.cell
def __(mo):
    mo.md(r"""# 1.2.1 オンライン実験による方策性能推定""")
    return
...
@app.cell
def __(mo):
    mo.md(r"""## オンライン実験で収集するデータのサイズ$n$を変化させたときのAVG推定量の平均二乗誤差の挙動""")
    return
...
def __(
    ...
    return dataset, result_df_datasize, se_df, se_df_list, se_list


@app.cell
def __(mo):
    mo.md(r"""## 報酬のノイズの大きさ$\sigma$を変化させたときのAVG推定量の平均二乗誤差の挙動""")
    return
...
def __(
    ...
    for noise in noise_list:
        ...
        ## 評価方策の真の性能(policy value)を近似するためのテストデータ
        test_bandit_data = dataset_noise.obtain_batch_bandit_feedback(n_rounds=test_data_size)
        ...
        for _ in tqdm(range(num_runs), desc=f"noise={noise}..."):
            ...
            online_experiment_data_noise = dataset_noise.obtain_batch_bandit_feedback(
                n_rounds=num_data_noise,
            )
...
@app.cell
def __(plt, result_df_datasize, result_df_noise, sns):
    ...
    fig
    return ax, ax_list, fig, i, result_df, title_dict, x_dict
