...
class RankingSyntheticBanditDataset:
    ...
    def obtain_batch_bandit_feedback(self, n_rounds: int) -> RankingBanditFeedback:
        ...
        ranking_candidates = list(
            itertools.permutations(range(self.n_actions), self.len_list)
        )
        ...
        position_level_expected_reward_ = self._calc_position_level_expected_reward(
            contexts
        )
    ...
    def _default_position_level_reward(self, contexts: np.ndarray) -> np.ndarray:
        ...
        if self.action_context is None:
            self.action_context = np.random.normal(
                loc=0, scale=1, size=(self.n_actions, self.dim_context)
            )
        ...
        for i in range(n_rounds):
            for k in range(self.len_list):
                position_level_reward[i * self.len_list + k] = np.dot(
                    contexts[i], self.action_context[k]
                )
